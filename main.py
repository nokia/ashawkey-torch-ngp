...
if __name__ == '__main__':
    ...
    ### testing options
    parser.add_argument('--save_cnt', type=int, default=0, help="save checkpoints for $ times during training")
    parser.add_argument('--eval_cnt', type=int, default=0, help="perform validation for $ times during training")
    ...
    ### training options
    parser.add_argument('--iters', type=int, default=2e10, help="training iters")
    parser.add_argument('--lr', type=float, default=4*1e-3, help="initial learning rate")
    ...
    parser.add_argument('--lambda_distort', type=float, default=0.0, help="loss scale (only for non-cuda-ray mode)")
    ...
    ### sampling options
    parser.add_argument('--hsm', action='store_true', help="activate hard sample mining method during training")
    parser.add_argument('--val_checkpoint', type=int, default=120, help="log validation set PSNR at (seconds) interval")
    parser.add_argument('--val_img', type=int, default=-1, help="plot a specific validation img, if -1 not in use")
    parser.add_argument('--max_training_time', type=int, default=4, help="maximum training time in minutes")
    parser.add_argument('--wandb', action='store_true', help="print stats to wandb")
    parser.add_argument('--vis_pointcloud', action='store_true', help="visualize the sample points")
    ...
    if opt.wandb:
        if opt.hsm:
            group = "Hard Sample Mining"
        else:
            group = "Baseline"

        dataset = opt.path.split("data/")[1].rstrip('/')
        
        wandb.init(project="Hard Sample Mining",
            name=f'{group}_{dataset}',
            group=group,
            config=opt)
        
        wandb.define_metric("training loss mean", step_metric="iteration")
        wandb.define_metric("training loss variance", step_metric="iteration")
        wandb.define_metric("PSNR", step_metric="training time")
        wandb.define_metric("LPIPS", step_metric="training time")
        wandb.define_metric("SSIM", step_metric="training time")
        wandb.define_metric("Running Max Memory Usage (MB)", step_metric="training time")
    ...
    if opt.O:
        ...
        opt.adaptive_num_rays = True # do not set automatically
    ...
    opt.criterion = criterion # in order to calculate proxy loss for the gradient norm approximation
    ...
    if opt.test:
        ...
        scheduler = torch.optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 1.0) # placeholder learning rate scheduler, TODO: make it based on wall-clock runtime
        ...
        if opt.gui:
            ...
            # if opt.val_img == -1: # not single image visualization
            # last validation
            trainer.metrics = [PSNRMeter(), SSIMMeter(), LPIPSMeter(device=device)]
